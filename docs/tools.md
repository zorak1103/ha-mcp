...
| `manage_area`         | Consolidated area management (actions: list, get, create, update, delete; update supports label_mode/alias_mode: add/remove/replace; get supports include_entities/include_automations; format: natural/json for list/get). `labels` must be an array of existing `label_id` strings - a non-array value or a non-string element is rejected outright, and an unknown id is rejected rather than silently dropped. |
...
| `manage_entity`       | Entity registry management (actions: get, update, delete; update fields: name, icon, area_id, disabled_by, hidden_by, labels, aliases, new_entity_id; update supports label_mode/alias_mode: add/remove/replace; format: natural/json). **Friendly-name hierarchy:** registry `name` > automation `alias` > auto-slug. `name=""` clears the registry override and falls back to the auto-slug (not the alias). `labels` must be an array of existing `label_id` strings - a non-array value or a non-string element is rejected outright, and an unknown id is rejected rather than silently dropped. |
| `manage_device`       | Device registry management (actions: get, update, delete; update fields: name_by_user, area_id, disabled_by, labels; update supports label_mode: add/remove/replace; format: natural/json). `labels` must be an array of existing `label_id` strings - a non-array value or a non-string element is rejected outright, and an unknown id is rejected rather than silently dropped. |
...
**Flexible ID Lookup**: The `scene_id` parameter accepts multiple formats for all actions:
- Entity ID: `scene.movie_time`
- Friendly name: `movie time` (case-insensitive partial match). `update`/`patch` refuse with an "ambiguous" error listing candidates if the identifier matches more than one scene — use the exact entity_id in that case.

`update` skips the write (and the reload it would trigger) when the merged config is identical to the stored one.
...
| Tool               | Description                                                                                          |
| ------------------ | ---------------------------------------------------------------------------------------------------- |
| `manage_trace`     | View automation and script execution traces (actions: list, get, debug; format: natural/json). `list` supports optional `wait=true` to poll until traces appear — useful immediately after triggering an automation (HA records traces asynchronously). If `entity_id` can't be resolved to HA's internal trace key (`list`/`get`), the response carries a distinct warning instead of the generic "not available yet" message, since retrying or `wait=true` won't help in that case. |
