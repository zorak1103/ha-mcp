...
| `manage_area`         | Consolidated area management (actions: list, get, create, update, delete; update supports label_mode/alias_mode: add/remove/replace; get supports include_entities/include_automations; format: natural/json for list/get). `labels` must be existing `label_id`s - validated pre-write, unknown ids are rejected rather than silently dropped. |
...
| `manage_entity`       | Entity registry management (actions: get, update, delete; update fields: name, icon, area_id, disabled_by, hidden_by, labels, aliases, new_entity_id; update supports label_mode/alias_mode: add/remove/replace; format: natural/json). **Friendly-name hierarchy:** registry `name` > automation `alias` > auto-slug. `name=""` clears the registry override and falls back to the auto-slug (not the alias). `labels` must be existing `label_id`s - validated pre-write, unknown ids are rejected rather than silently dropped. |
| `manage_device`       | Device registry management (actions: get, update, delete; update fields: name_by_user, area_id, disabled_by, labels; update supports label_mode: add/remove/replace; format: natural/json). `labels` must be existing `label_id`s - validated pre-write, unknown ids are rejected rather than silently dropped. |
