...
- **`manage_statistics:clear`**: use `parseStatisticIDsStrict` (`statistics.go`), not the read path's lenient `parseStatisticIDs` (`entities_consolidated.go`) - the latter silently drops bad elements, which is wrong for an irreversible purge. Also not matched by a `manage_*:delete` filter glob (action name is `clear`, not `delete`) - block `manage_statistics:clear` explicitly if a policy needs it excluded.
