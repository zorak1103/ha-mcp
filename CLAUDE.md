...
- **`manage_helper` gained 15 `template_*` subtypes (issue #206) via a table-driven design**: `helpers_template_types.go`'s `templateSubtypes` map (one entry per HA template domain, each a list of `templateField`s) drives `buildTemplateHelperConfig(typeName)`; `templateHelperTypes()`/`templateHelperProperties()`/`templateSubtypeNames()` derive the schema, builder registration, and type-name list from the same source so they can't drift apart.
- **`readTemplateField` renames the config key uniformly after every `argReader` call, not per-`kind`** - a per-kind switch would forget the rename for non-string kinds, which an earlier draft did.
...
- **`argReader.actionValue()` bounds an HA `ActionSelector` value by depth/node-count (`maxActionDepth`=16/`maxActionNodes`=2000) AND per-string/key byte length (`maxScalarStringLen`) instead of by shape** - action fields nest freely and HA validates the semantics itself; same "bound what can't be shape-validated" pattern as `filter`'s duration-dict handling. The byte-length bound is not optional: depth/node count alone never catches `{"action": "<50MB string>"}` - that's only 2 nodes at depth 2 - so an earlier version missing it violated this file's own stated invariant that every string-shaped leaf value, at any nesting level, must be length-checked before being stored. `maxActionDepth` started at 8 and was raised to 16: a routine `choose` action (list → option map → conditions/sequence lists → action map → target map → entity_id list → string) is already depth 9, so 8 rejected ordinary automation-shaped actions outright, not just adversarial ones - depth alone is a weak signal here, node-count and string-length do the real bounding.
- **`templateSubtype.inclusivePairs` must be checked against each field's resolved `configKey()`, not its caller-facing `arg` name** - `readTemplateField` renames every field (`config[f.arg]` → `config[f.configKey()]`) before `checkInclusivePairs` runs, so comparing `config[pair[0]]`/`config[pair[1]]` using the pair's `arg` strings (as an early version did) always misses: `template_cover`'s `open`/`close` pair renames to `open_cover`/`close_cover`, and the presence check against `open`/`close` finds neither, ever - the vol.Inclusive validation silently never fires. `subtypeConfigKey()` resolves each pair element to the field's own `configKey()` before the presence check.
- **`resolveTemplateFieldsForDomain`'s single-owner fallback field must be dropped, not merely sorted after, when its config key collides with one of the domain's own matched fields** - `template_alarm_control_panel`'s bare `code_format` is single-owner (only one subtype declares that arg name), so it survives the ambiguity check even for domain `"lock"`, where it collides with `template_lock`'s own `lock_code_format` (haKey `code_format`). Relying on sort order alone (an earlier version's fix) means a caller who updates a lock via the plain `code_format` arg name gets it written straight through instead of reported as unresolved - defeating the rename `lock_code_format` exists for. `matchedConfigKeys` (the domain's own matched fields' resolved keys) is checked before keeping a fallback field.
...
- **Template subtype `device_class` update support is per-type, not a flat "4 create-only, 11 never" split** - verify each type's OPTIONS_FLOW schema individually rather than assuming CONFIG_FLOW support implies (or excludes) OPTIONS_FLOW support. HA's `generate_schema` (`template/config_flow.py`) guards `device_class` behind `if flow_type == "config":` for `button`/`cover`/`event`/`update` (create-only, `perTypeDeviceClassSupport` minus `perTypeDeviceClassUpdateSupport`), but declares it unconditionally for `number` (both flows - `template_number` is the one type in `perTypeDeviceClassUpdateSupport`). An earlier version of this fix gated ALL template domains' `device_class` update read on `templateSubtypeDomains[entryCtx.entityDomain]` alone, which silently broke `template_number`'s `device_class` update entirely (excluded, when HA actually accepts it) while "fixing" the other 4. `buildConfigEntryUpdateConfig`'s generic `r.str("device_class")` read is now gated by `deviceClassSupportedOnTemplateUpdate(entryCtx.entityDomain)` (`helpers_template_types.go`), and `perTypeUpdateExcludedFields` only excludes `device_class` for a type when it is NOT in `perTypeDeviceClassUpdateSupport` - both derive from the same two tables so they can't drift from each other, but adding a 16th template subtype still requires checking its real OPTIONS_FLOW schema rather than copying the existing pattern. `addTemplateConfigEntryUpdateFields` never reads `device_class` at all (by design, see its own doc comment) - only the generic read needed this fix.
