...
- **`generic_thermostat`'s config/options flow has a trailing `presets` step** (`PREVENT_EXTRA` schema) - handled generically by the schema-driven flow engine (see "Config Entry Flow Multi-Step Pattern" above), not a per-platform special case.
- **generic_thermostat's six preset fields** (`away_temp`/`eco_temp`/`home_temp`/`comfort_temp`/`sleep_temp`/`activity_temp`) are real, settable `manage_helper` fields, generated from one shared list (`genericThermostatPresets`, `helpers_config_builders_extended.go`) that both the create/update builders and the schema (`genericThermostatPresetSchemaProperties`) derive from - don't hand-duplicate the two again. `addGenericThermostatPresetFields` reads them only when entity domain == `"climate"`, pinned by `TestThermostatEntityDomain_IsUniqueAcrossHelperTypes` (the gate assumes single ownership of that domain). HA does not clamp presets to `min_temp`/`max_temp`, and a preset can't be cleared once set.
- **`manage_helper update`'s success message reports only fields that actually reached the payload** (`updateSuccessMessage`/`splitAppliedFields`, snapshotted *before* `client.UpdateHelper` runs) - not every non-dispatch arg the caller passed. A field dropped by a platform gate (like the one above) must be reported as ignored, not applied. `splitAppliedFields` resolves the arg name via `resolveUpdateConfigKey` (which checks `resolveTemplateFieldsForDomain` first, then falls back to the static `updateConfigKeyAliases` map, e.g. `heater_entity_id` → `heater`) before checking `appliedKeys` - comparing the raw arg name directly reports every renamed field as ignored even when HA accepted it. A static alias map alone can't express a template subtype's renames (`open`→`open_cover` only for `template_cover`, stays bare for `template_lock`) since the same arg name renames differently per domain - that's why the domain-aware resolver is checked first.
...
- **`checkHelperOnlyDomain` resolves the real registry platform and rejects unless it's one of the platforms `widenedHelperOnlyDomains` (derived from `helperTypes`' `validEntityDomains`) allows for that domain; a registry fetch failure or not-found entity hard-fails rather than degrading to unchecked, since degrading is exactly the gap being closed. Deliberately scoped to only the 16 newly-widened domains** (`newlyWidenedHelperDomains`, itself derived from `helperTypes`' `validEntityDomains` minus the hand-maintained `preExistingHelperOnlyDomains` set, so a future helper type touching a new domain is covered automatically) **, not the older `sensor`/`binary_sensor`/`climate`/`humidifier`/`select` ambiguity that predates this branch. `manage_helper`'s `delete` action shares the same call** (`checkHelperOnlyDomain` in `handleDelete`, mirroring `handleUpdate`) **- both actions are covered.
...
- **`sortedHelperTypeNames()`/`buildHelperConfigBuildersRegistry()`/`buildHelperTypesRegistry()`/`buildPerTypeUpdateExcludedFields()` wrap what were plain package-level literals** so their initializers can merge in `helpers_template_types.go`'s data - a bare `func init()` fails `gochecknoinits`, a `var _ = ...()` side-effect trick fails `unused`; wrapping the literal in a same-named `build*`/`sorted*` func (`var x = buildX()`) lets Go's var dependency analysis order initialization correctly regardless of which file declares what.
- **`argReader.actionValue()` bounds an HA `ActionSelector` value by depth/node-count (`maxActionDepth`=16/`maxActionNodes`=2000) AND per-string/key byte length (`maxScalarStringLen`), not by shape** - action fields nest freely and HA validates the semantics itself, so depth/node-count alone never catches an oversized leaf (`{"action": "<50MB string>"}` is only 2 nodes at depth 2) - every string-shaped leaf must be length-checked regardless of nesting. `maxActionDepth`=16 because a routine `choose` action (list → option map → conditions/sequence lists → action map → target map → entity_id list → string) already reaches depth 9 - depth alone is a weak signal, node-count and string-length do the real bounding.
- **`templateSubtype.inclusivePairs` must be checked against each field's resolved `configKey()`, not its caller-facing `arg` name** - `readTemplateField` renames every field (`config[f.arg]` → `config[f.configKey()]`) before `checkInclusivePairs` runs, so comparing using the pair's `arg` strings always misses: `template_cover`'s `open`/`close` pair renames to `open_cover`/`close_cover`, and a presence check against `open`/`close` finds neither, ever - the vol.Inclusive validation silently never fires. `subtypeConfigKey()` resolves each pair element to the field's own `configKey()` before the presence check.
- **`resolveTemplateFieldsForDomain`'s single-owner fallback field must be dropped, not merely sorted after, when its config key collides with one of the domain's own matched fields** - `template_alarm_control_panel`'s bare `code_format` is single-owner (only one subtype declares that arg name), so it survives the ambiguity check even for domain `"lock"`, where it collides with `template_lock`'s own `lock_code_format` (haKey `code_format`). Sort order alone isn't enough: a caller updating a lock via the plain `code_format` arg would get it written straight through instead of reported as unresolved, defeating the rename `lock_code_format` exists for. `matchedConfigKeys` (the domain's own matched fields' resolved keys) is checked before keeping a fallback field.
- **`resolveTemplateFieldsForDomain` is memoized per domain** (`templateFieldsForDomainCache`, mutex-guarded) since `templateSubtypes` is immutable after init and the function was being fully rebuilt - three maps, two sorts over ~90 fields - on every call, including once per arg inside `splitAppliedFields`'s loop via `resolveUpdateConfigKey`. The returned slice is shared across callers; treat it as read-only.
- **`resolveTemplateFieldsForDomain`'s returned slice must be sorted by arg name, not left in map-range order**: `firstByArg`/`matchedByArg` are maps, and two different arg names can resolve to the *same* HA config key for one domain (`template_lock`'s own `lock_code_format`→`code_format` and `template_alarm_control_panel`'s unrelated, unambiguous `code_format` both apply to a `lock`-domain update) - `addTemplateConfigEntryUpdateFields` processes the slice in order and the last write to that key wins, so an unsorted order made the winner (and thus which field's value survived) vary from one call to the next, not just one process to the next, since Go's map iteration order is randomized per range statement. `templateHelperProperties()` has the same class of bug for its "first description wins" dedup over `templateSubtypes` (a shared arg like `turn_on`/`open`/`temperature` has a different `desc` per subtype) - both now iterate a sorted key list.
- **Template subtype `device_class` update support is per-type, not a flat "4 create-only, 11 never" split** - verify each type's OPTIONS_FLOW schema individually rather than assuming CONFIG_FLOW support implies (or excludes) OPTIONS_FLOW support. HA's `generate_schema` (`template/config_flow.py`) guards `device_class` behind `if flow_type == "config":` for `button`/`cover`/`event`/`update` (create-only, `perTypeDeviceClassSupport` minus `perTypeDeviceClassUpdateSupport`), but declares it unconditionally for `number` (both flows - `template_number` is the one type in `perTypeDeviceClassUpdateSupport`). `buildConfigEntryUpdateConfig`'s generic `r.str("device_class")` read is gated by `deviceClassSupportedOnTemplateUpdate(entryCtx.entityDomain)` (`helpers_template_types.go`), and `perTypeUpdateExcludedFields` only excludes `device_class` for a type when it is NOT in `perTypeDeviceClassUpdateSupport` - both derive from the same two tables so they can't drift, but adding a 16th template subtype still requires checking its real OPTIONS_FLOW schema rather than copying the existing pattern. `addTemplateConfigEntryUpdateFields` never reads `device_class` at all (by design, see its own doc comment) - only the generic read needed this gate.
- **`switch_as_x`'s entity_id cannot be predicted from `name`, unlike every other Config Entry helper** - its flow schema has no `name` field at all; HA derives the wrapper's name from the wrapped source switch (`entity.py`'s `BaseEntity.__init__` reads `wrapped_switch.original_name`, and copies `has_entity_name`, which can add a device-name prefix). `CreateHelperEntity` (`hybrid_client.go`) resolves the real id via the entity registry, keyed by the created config entry's `entry_id` and filtered to the platform's expected entity domain (`WaitForConfigEntryEntity`'s `preferDomain`, via `determineEntityDomainForConfigEntry`) - a config entry can register more than one entity (a tariffed `utility_meter` registers a `select.*` tariff selector plus a `sensor.*` per tariff, select first), so an unfiltered "first match wins" could silently resolve to the wrong domain. `entityIDPredictable()` (mirrored in both `hybrid_client.go` and `helpers_consolidated.go`) gates the old name-based prediction to platforms other than `switch_as_x`: since that guess (`<target_domain>.<slugified name>`) is caller-controlled and may name an unrelated, pre-existing entity, it is never used as a write target (icon updates) or reported as fact when resolution can't confirm an id in time - `createConfigEntryHelper` returns an `entityUnresolvedError` (rendered as a success-with-WARNING, the helper still exists) instead.
- **`RESTClient`'s `http.Client` must never leave `Transport` nil** - a nil `Transport` implicitly falls back to the shared, package-global `http.DefaultTransport`, and `httptest.Server.Close()` unconditionally calls `http.DefaultTransport.CloseIdleConnections()` as a courtesy side effect (stdlib `net/http/httptest/server.go`) - not scoped to its own connections. Under `t.Parallel()` REST-client tests (each with its own `httptest.NewServer`), one subtest's teardown can silently kill another still-in-flight subtest's connection, surfacing as a flaky `net/http: HTTP/1.x transport connection broken` error (issue #227). `NewRESTClientWithConfig` sets `Transport: http.DefaultTransport.(*http.Transport).Clone()` so every client gets an isolated connection pool immune to unrelated `CloseIdleConnections()` calls anywhere in the process.
- **`HelperFormatter`'s two implementations disagreed on `helperType`** (issue #216): `JSONHelperFormatter.FormatHelperDetail` ignores the parameter and marshals whatever `buildHelperDetails` built, while `NaturalHelperFormatter.FormatHelperDetail` switches on it - so a domain reaching the formatter without a case succeeded in JSON (with a near-empty map) and errored in natural. `climate`/`humidifier`/`select` and the 15 `template_*` subtype domains hit exactly this: `buildHelperDetails` also had no case for them, so even JSON only returned `entity_id`/`state`/`friendly_name`. Both now have a generic `default:` - `addGenericDetails` writes `state.Attributes` (minus `genericDetailSkipAttributes`: `friendly_name`, `supported_features`, `attribution`) through `putDetail`, and `formatGenericDetail` renders the result sorted by key (map iteration order is randomized per range statement - see the `resolveTemplateFieldsForDomain` entry above for the same class of bug). `putDetail` (`helpers_consolidated.go`) is the collision guard a group helper's entity needs: HA's group platform reports its member list as an `entity_id` *attribute*, which would otherwise silently overwrite the real entity_id this detail map is keyed on - `putDetail` renames that one case to `members` (mirroring `addGroupDetails`' own convention) and routes any other collision to `attr_<key>` instead of overwriting. `formatDetailValue` (`formatter/util.go`) additionally sanitizes newlines out of attribute values (an attacker-controlled value must not be able to forge extra `Key: value` lines the way `sanitizeDisplayName` already prevents for entity names) and caps natural-format output only - long lists get a `… +N more` suffix and long strings truncate at `maxDetailValueChars`; JSON stays complete. `buildHelperDetails` and `NaturalHelperFormatter.FormatHelperDetail` are now both map registries (`helperDetailBuilders`/`dedicatedDetailRenderers`) instead of hand-written switches, and `TestHelperDetailBuilders_MatchDedicatedRenderers` pins their key sets equal - a new *dedicated* renderer (like the 9 hand-tuned ones) still requires adding to both registries, but drift is now a test failure instead of a silent runtime split. A third hand-maintained domain list had the same drift independently: `handleGetDetails`' dispatch listed helper domains by hand and missed `switch_as_x`'s `siren`/`valve` targets, so `get_details` on those entities errored even though `update`/`delete` worked. Replaced by `helperEntityDomains`, derived from every `helperTypes` entry's `validEntityDomains` (same pattern as `templateSubtypeDomains`) - a new helper domain needs no dispatch-list edit at all now.
