...
- `manage_statistics` — actions: list, validate, clear. WebSocket-only recorder ops (`recorder/list_statistic_ids` / `recorder/validate_statistics` / `recorder/clear_statistics`) over raw statistic_ids, which exist independently of the entity registry — this is the orphaned-statistics workflow; `validate` reproduces Developer Tools → Statistics, where issue type `no_state` marks an id whose backing entity is gone. `statistic_type` (mean|sum) is passed to recorder/list_statistic_ids; `limit`/`offset`/`cursor` pagination for list/validate is client-side, via the shared `ParsePaginationParams`/`ApplyPagination` helpers (`pagination.go`) — `buildStatisticListPaginationParams` (`statistics.go`) applies `defaultStatisticListLimit` only when the caller supplied neither `limit` nor `cursor` (an omitted limit must not mean unlimited, and a cursor's own stored limit must not be overridden), and `validateLimitArg` rejects a limit before it reaches pagination so a non-finite or out-of-int-range value (e.g. `1e19`) can't overflow `int` and silently defeat the cap. `clear` requires `statistic_ids`; HA waits up to ~10s for the recorder queue to acknowledge before responding. `dry_run` is parsed strictly (`parseBoolArg`) — a non-bool value errors rather than silently defaulting to `false`, since MCP does no schema validation of its own and this flag is the only thing between a preview and an irreversible purge. No caching (recorder DB is volatile). Handler: `internal/handlers/statistics.go`. Response-type services like `recorder.get_statistics` return service response data rather than state changes; call them through `call_service` with `return_response: true`
...
**Implementation**: `ToolFilterEngine` in `internal/mcp/tool_filter.go` with static classification in `internal/mcp/access_control.go` (38 tools, read/write per action). `ValidateFilterConfig()` runs at startup before engine creation — rejects unknown tools, typos, and removed sub-actions with a combined error listing all problems at once (server refuses to start). Three-stage enforcement: startup config validation → registry filtering (removes/modifies tools) → runtime check (blocks calls).
...
- `call_service`: ordinary calls snapshot target entities before, poll for state changes after, and append "\nState changes: entity: old → new" or a warning; `return_response: true` calls skip Smart Wait, return the complete service payload, and set no-polling metadata/caveat because response-capable services may still mutate state
...
- **`get_details` deliberately does NOT call `checkHelperOnlyDomain`**, unlike `update`/`delete` - the guard bounds write blast radius (mutating/deleting a real integration's entity), which a read has none of. Wiring it in would newly reject `get_details` on ordinary `light`/`cover`/`fan`/`lock` entities that work today (`widenedHelperOnlyDomains` covers more than just whichever domain a given review round flagged) and would hard-fail a read on a transient registry-fetch error. Raised and rejected twice now (issue #216 review rounds) - don't re-add it without solving both regressions first.
...
- **`HelperFormatter`'s two implementations disagreed on `helperType`** (issue #216): `JSONHelperFormatter.FormatHelperDetail` ignores the parameter and marshals whatever `buildHelperDetails` built, while `NaturalHelperFormatter.FormatHelperDetail` switches on it - so a domain reaching the formatter without a case succeeded in JSON (with a near-empty map) and errored in natural. `climate`/`humidifier`/`select` and the 15 `template_*` subtype domains hit exactly this: `buildHelperDetails` also had no case for them, so even JSON only returned `entity_id`/`state`/`friendly_name`. Both now have a generic `default:` - `addGenericDetails` writes `state.Attributes` (minus `genericDetailSkipAttributes`: `friendly_name`, `supported_features`, `attribution`) through `putDetail`, and `formatGenericDetail` renders the result sorted by key (map iteration order is randomized per range statement - see the `resolveTemplateFieldsForDomain` entry above for the same class of bug). `putDetail` (`helpers_consolidated.go`) is the collision guard a group helper's entity needs: HA's group platform reports its member list as an `entity_id` *attribute*, which would otherwise silently overwrite the real entity_id this detail map is keyed on - `putDetail` renames that one case to `members` (mirroring `addGroupDetails`' own convention) and routes any other collision to `attr_<key>` instead of overwriting. `formatter.FormatDetailValue` (`formatter/util.go`, exported for reuse outside the package - see the `manage_statistics` gotcha below) additionally sanitizes newlines out of attribute values (an attacker-controlled value must not be able to forge extra `Key: value` lines the way `formatter.SanitizeDisplayName` already prevents for entity names) and caps natural-format output only - long lists get a `… +N more` suffix and long strings truncate at `maxDetailValueChars`; JSON stays complete. `buildHelperDetails` and `NaturalHelperFormatter.FormatHelperDetail` are now both map registries (`helperDetailBuilders`/`dedicatedDetailRenderers`) instead of hand-written switches, and `TestHelperDetailBuilders_MatchDedicatedRenderers` pins their key sets equal - a new *dedicated* renderer (like the 9 hand-tuned ones) still requires adding to both registries, but drift is now a test failure instead of a silent runtime split. A third hand-maintained domain list had the same drift independently: `handleGetDetails`' dispatch listed helper domains by hand and missed `switch_as_x`'s `siren`/`valve` targets, so `get_details` on those entities errored even though `update`/`delete` worked. Replaced by `helperEntityDomains`, derived from every `helperTypes` entry's `validEntityDomains` (same pattern as `templateSubtypeDomains`) - a new helper domain needs no dispatch-list edit at all now.
- **`recorder/list_statistic_ids` merges integration-provided metadata with `statistics_meta` rows** - an id can be listed with zero recorded data, and an id whose metadata comes from an integration fallback (not the DB) is still listed after `recorder/clear_statistics`, which only purges DB rows + metadata. Don't assert list-absence after a clear, and don't treat listed-but-dataless ids as a bug (pinned by `TestManageStatisticsLifecycle`'s comments).
- **`manage_statistics:clear`**: use `parseStatisticIDsStrict` (`statistics.go`), not the read path's lenient `parseStatisticIDsLenient` (`entities_consolidated.go`) - the latter silently drops bad elements, which is wrong for an irreversible purge. Also not matched by a `manage_*:delete` filter glob (action name is `clear`, not `delete`) - block `manage_statistics:clear` explicitly if a policy needs it excluded. `clear` also cross-checks requested ids against `recorder/list_statistic_ids` before purging (`resolveKnownStatisticIDs`) so the success message can report which ids the recorder actually had vs. skipped, and supports `dry_run: true` to preview that split without purging - a pre-check fetch failure degrades to the pre-check-less behavior with an appended `WARNING:` line rather than blocking the clear. `parseStatisticIDsStrict` splits ids into `valid` (matches HA's statistic_id shape) and `unrecognized` (doesn't) rather than hard-failing the whole batch on a single shape mismatch - an orphaned statistic can carry an id shape older or buggier than HA's own validator currently allows, and cleaning that up is this tool's purpose; `resolveKnownStatisticIDs` against the recorder's real id list is the actual correctness gate, not the regex. Only when *every* id is unrecognized does the call still error, since nothing would be left to act on. Every element is also bounded by `maxScalarStringLen` - `maxClearStatisticIDs` only bounds element count, not length.
