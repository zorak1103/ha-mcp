...
**Config Entry Flow Multi-Step Pattern**: Some Config Entry platforms require multiple form submissions beyond an initial menu step. This is now **schema-driven, not per-platform hardcoded**: `internal/homeassistant/flow_steps.go`'s `buildStepSubmission()` treats each step's own `DataSchema` as the allow-list — a caller-supplied field is routed into whichever step's schema declares it (via `indexStepSchema()`/`placementOf()`), and `createHelperViaConfigFlow()`/`updateHelperViaOptionsFlow()`'s `runOptionsFlowSteps()` loop submit steps until `create_entry`/`abort`/a step cap. A `consumed map[string]bool`, threaded through the whole loop and seeded by `seedConsumedRoutingKeys()` (routing-only keys like `template_type`/`target_domain`/`state_characteristic`/`icon` that are never a real submission value), tracks which caller fields have already been claimed — a field no step ever claims is reported via `unconsumedUserFields()`, checked only **after** every step has had a chance (this is what lets a field belonging to a later step, e.g. generic_thermostat's presets, be routed correctly instead of rejected against the first step's schema alone).
...
- **Template helper type field**: `type` field in template config determines sensor vs binary_sensor subtype for Config Entry Flow menu selection, but must be filtered by `shouldSkipConfigField` before API submission. The actual config key the `template_sensor`/`template_binary_sensor` builders write is `template_type` (`buildTemplateSensorConfig`/`buildTemplateBinarySensorConfig`, `internal/handlers/helpers_consolidated.go`) — `determineTemplateSubtype` (`internal/homeassistant/hybrid_client.go`) previously read a dead key, `platformTemplate_type` (residue of a bad global `template_` → `platformTemplate_` rename), so it never found the value and fell through to `device_class` inference; `manage_helper create type=template_binary_sensor` without a binary-inferrable `device_class` silently created a **sensor** while reporting a `binary_sensor.*` entity id that didn't exist (issue #211). `determineTemplateSubtype` is read by two call sites — the create-time menu-step selector and `predictEntityIDForConfigEntry`'s icon-write target — so a key mismatch here breaks both at once. If you rename the string literal these builders write, grep for it; nothing enforces the two sides agree.
...
- **`generic_thermostat` create/update both fail against HA's newer two-step config flow (`user`/`init` → `presets`)** (issue #194): HA core's `generic_thermostat/config_flow.py` added a trailing `presets` step (`PRESETS_SCHEMA`, all-Optional preset-temperature fields we don't expose as tool parameters) to both `CONFIG_FLOW` and `OPTIONS_FLOW`. That schema is `PREVENT_EXTRA`, so resubmitting the full config there - `buildConfigForFlowStep`'s `default:` behavior, since `generic_thermostat` had no case - fails with `"extra keys not allowed @ data['ac_mode']"` etc. This had been mis-attributed in this file as a generic "HA version-specific 400 error" (see the pre-existing-integration-test-failures note above, now corrected) rather than root-caused. Fixed on **create** by adding a `platformGenericThermostat` case (delegating to `buildGenericThermostatStepConfig`, mirroring how `platformFilter` delegates to `buildFilterStepConfig`) that returns an empty map for `stepID == "presets"` - the create loop already resubmits per-step generically, so no other change was needed there. **Update** needed a separate fix: `updateHelperViaOptionsFlow` submitted once and hard-failed on anything but `create_entry`, so every `generic_thermostat` update (previously untested - no update test existed) hit `"unexpected options flow result type: form"`, since the flow always advances `init` → `presets`. Fixed via `submitOptionsFlowPresetsStep()`, gated on HA's own `StepID` (not `config.Platform`: on the update path `HelperConfig.Platform` is the entity **domain** (`"climate"`), not the helper type (`"generic_thermostat"`) - `ParseHelperEntityID` only recovers the domain, see the `min_max_type`/`resolveConfigEntryPlatformForMinMaxType` gotcha above for the same distinction). `TestGenericThermostatLifecycle` (`internal/handlers/integration/generic_thermostat_integration_test.go`) now includes an update step, confirmed passing against live HA.
  ...
  - **Superseded**: the platform-specific functions named throughout this entry (`buildConfigForFlowStep`, `buildGenericThermostatStepConfig`, `submitOptionsFlowPresetsStep`, `restrictToSchemaFields`, `mergeOptionsFlowConfig`, `normalizeOptionsFlowDurations`, `filterStepFields`) no longer exist - the reasoning here (the create/update omitted-optional-key asymmetry, the stable-flow-id abort convention, surfacing HA's validation errors) still holds and now lives in the generic schema-driven engine (`internal/homeassistant/flow_steps.go`, see "Config Entry Flow Multi-Step Pattern" above).
- **`manage_helper` gained 15 new `template_*` subtypes (issue #206) via a table-driven design instead of 15 hand-written builders:** `internal/handlers/helpers_template_types.go`'s `templateSubtypes` map has one entry per HA template domain (`alarm_control_panel`, `button`, `cover`, `device_tracker`, `event`, `fan`, `image`, `light`, `lock`, `number`, `select`, `switch`, `update`, `vacuum`, `weather`), each a list of `templateField`s (`arg` the tool-facing name, `haKey` the real HA config key when it differs, `kind` selecting which `argReader` method reads it, `required`). A single `buildTemplateHelperConfig(typeName)` generates every subtype's create builder from that table; `templateHelperTypes()`/`templateHelperProperties()`/`templateSubtypeNames()` derive `helperTypeMetadata`, the tool's JSON schema properties, and the type-name list from the same source, so the field list can't drift between schema, builder, and validation the way three independently maintained versions could.
  - **`readTemplateField`'s rename-after-read has to be generic, not per-`kind`:** every `argReader` method (`str`/`num`/`boolean`/`strSlice`/`actionValue`) writes to `config[f.arg]`; only `str`/`strID` have a built-in "As" rename variant, so `readTemplateField` renames `config[f.arg]` to `f.configKey()` uniformly after calling whichever reader `kind` selects, rather than each `case` in a per-kind switch remembering to do it - the first version of this function forgot the rename for non-string kinds, which is exactly the class of bug a uniform post-read step forecloses.
  - **`resolveTemplateFieldsForDomain` resolves a real ambiguity, not a hypothetical one:** three arg names - `state`, `open`, `stop` - are declared by multiple subtypes with *different* HA keys (e.g. cover's `open` → `open_cover`, lock's `open` → bare `open`; template_alarm_control_panel/template_switch's `state` → `value_template`, everything else's `state` → bare `state`). Create has a per-subtype call site (`buildTemplateHelperConfig(typeName)`) so this never matters there, but the shared one-size-fits-all update builder (`addExtendedConfigEntryFields` in `helpers_config_builders_extended.go`) has no equivalent per-subtype dispatch - it resolves the ambiguity by matching the entity's actual domain (`entryCtx.entityDomain`) and, when no subtype's domain matches (e.g. a `template_sensor` update, domain `sensor`, matches none of the 15 subtypes), **skips** the ambiguous field entirely rather than picking whichever HA key Go's map iteration happened to yield first - the failure mode this replaces would have intermittently renamed `template_sensor`'s own `state` field to `value_template` or worse.
- **`ParseHelperEntityID`'s hardcoded `HelperPlatforms` list didn't cover any of the 15 new template subtypes' entity domains, silently breaking `manage_helper update`/`delete` for all of them despite `create` working fine:** `manage_helper update`/`delete` (`internal/handlers/helper_common.go`) only recognize entity domains present in this list; discovered via the live-HA integration sweep, not by inspection. Fixed by adding `alarm_control_panel`, `button`, `cover`, `device_tracker`, `event`, `fan`, `image`, `light`, `lock`, `number`, `switch`, `update`, `vacuum`, `weather`, plus `siren`/`valve` to complete the `switch_as_x` target-domain set (see below).
  - **Incidentally fixed a pre-existing, previously undiscovered gap: `switch_as_x` helpers targeting `cover`/`fan`/`light`/`lock` were never updatable/deletable via `manage_helper` either, and `siren`/`valve` still wouldn't have been without the two extra additions:** `helperTypes["switch_as_x"].validEntityDomains` (`internal/handlers/helpers_consolidated.go`) is `["cover", "fan", "light", "lock", "siren", "valve"]`, none of which were in `HelperPlatforms` before this fix (only `select` had been added, for an earlier Config Entry helper) - so `manage_helper update`/`delete` against any `switch_as_x` helper other than one targeting `light` (the type's `entityPrefix` default) silently failed with "invalid entity_id format," unrelated to and unnoticed until this template-subtype sweep touched the same list.
- **HA template domain config keys don't follow a consistent `<name>_action` convention - several looked like they should but don't, and an earlier draft of `templateSubtypes` guessed wrong:** cover's open/close/stop/set_position actions are `open_cover`/`close_cover`/`stop_cover`/`set_cover_position`, not `open_action` etc.; alarm_control_panel's `arm_away`/`disarm`/`trigger` fields are bare, no `_action` suffix at all; fan/light's `turn_on`/`turn_off`/`set_percentage`/`set_level`/`set_hs`/`set_temperature` are also bare, matching the tool's own arg names exactly; vacuum's `fan_speed_list` tool arg maps to HA key `fan_speeds` (plural, not `fan_speed_list`). All verified against HA core's actual `template/{domain}.py` source (github.com/home-assistant/core), not guessed - the guessed-convention draft was wrong on several of these and only caught by the live-HA integration sweep. A future contributor extending `templateSubtypes` should verify each new `haKey` against HA source rather than assume a naming pattern.
- **Two tool-arg naming collisions were deliberately resolved by renaming the tool arg, not the HA key:** `select`'s `options` field was renamed to `options_template` at the tool-arg level to avoid colliding with `input_select`'s `options` (a fixed string array - semantically different from `template_select`'s Jinja-templated options list); `code_format` collides in HA itself between `alarm_control_panel` (fixed enum: `number`/`text`) and `lock` (freeform template) - `template_lock`'s tool arg is `lock_code_format` (mapped via `haKey: "code_format"`) so the schema doesn't need one property name to support two incompatible shapes depending on which subtype is active.
- **`sortedHelperTypeNames()`/`buildHelperConfigBuildersRegistry()`/`buildHelperTypesRegistry()`/`buildPerTypeUpdateExcludedFields()` wrap what used to be plain package-level map/slice literals so their initializers can merge in `helpers_template_types.go`'s template-subtype data:** a literal `func init()` was rejected by `gochecknoinits`; a `var _ = someFunc()` side-effect trick was tried next and flagged by `unused`. Wrapping each var's literal in a same-named `build*`/`sorted*` function and assigning `var x = buildX()` sidesteps both - Go's package-level var dependency analysis (not file declaration order) resolves the initialization order correctly regardless of which file `templateSubtypes`/`templateHelperTypes()` live in.
- **`argReader.actionValue()` bounds an HA `ActionSelector` value by depth/node-count instead of by shape, unlike every other `argReader` method:** template subtypes' action-typed fields (e.g. `template_switch`'s `turn_on`, `template_cover`'s `open`) accept a single action object (`{"action": "switch.turn_on", "target": {...}}`) or a list of them - `target`/`data` nest freely and HA validates the actual semantics itself, so there's no fixed shape to type-check the way `num`/`boolean`/`strSlice` do. `internal/handlers/helpers_arg_reader.go`'s `boundedActionShape()` walks the value recursively and rejects it past `maxActionDepth`(8)/`maxActionNodes`(2000), the same "bound what can't be shape-validated" pattern `raw()`'s duration-dict handling already used for `filter`'s `window_size`.
