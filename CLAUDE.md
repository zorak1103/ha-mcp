...
- **That same `HelperPlatforms` widening made a real, non-helper entity in one of those 16 domains (`light.hue_office`, a Zigbee `switch.*`, ...) indistinguishable from a helper to `ParseHelperEntityID`**, which matches on domain prefix alone. Before `checkHelperOnlyDomain` (`helpers_consolidated.go`), `handleUpdate` had no other gate, so such an entity would reach `client.UpdateHelper`, which routes any entity with a non-empty `config_entry_id` (every real integration entity has one) to an Options Flow submission against *that entity's own config entry* - silently editing/reloading a real integration instead of a helper. `checkHelperOnlyDomain` resolves the real registry platform and rejects unless it's one of the platforms `widenedHelperOnlyDomains` (derived from `helperTypes`' `validEntityDomains`) allows for that domain; a registry fetch failure or not-found entity hard-fails rather than degrading to unchecked, since degrading is exactly the gap being closed. Deliberately scoped to only the 16 newly-widened domains, not the older `sensor`/`binary_sensor`/`climate`/`humidifier`/`select` ambiguity that predates this branch. `manage_helper`'s `delete` action has the same underlying gap and was **not** fixed by this - see issue #217.
