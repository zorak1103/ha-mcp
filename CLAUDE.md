...
- **`switch_as_x`'s entity_id cannot be predicted from `name`, unlike every other Config Entry helper** - its flow schema has no `name` field at all; HA derives the wrapper's name from the wrapped source switch (`entity.py`'s `BaseEntity.__init__` reads `wrapped_switch.original_name`, and copies `has_entity_name`, which can add a device-name prefix). `CreateHelperEntity` (`hybrid_client.go`) resolves the real id via the entity registry, keyed by the created config entry's `entry_id` (`WaitForConfigEntryEntity`), instead of guessing; `createConfigEntryHelper` (`helpers_consolidated.go`) falls back to the old name-based prediction only if that lookup times out.
