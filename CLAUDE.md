...
- **`argReader.actionValue()` bounds an HA `ActionSelector` value by depth/node-count (`maxActionDepth`=8/`maxActionNodes`=2000) instead of by shape** - action fields nest freely and HA validates the semantics itself; same "bound what can't be shape-validated" pattern as `filter`'s duration-dict handling.
...
- **Template subtype `device_class` must never be read on update, not even for the 4 types that accept it on create** (`template_button`/`cover`/`event`/`update`, `perTypeDeviceClassSupport`): `perTypeUpdateExcludedFields` already documents it as OPTIONS_FLOW-unsupported for all 15 subtypes, but two code paths read it into `config` regardless of that table - `buildConfigEntryUpdateConfig`'s own generic `r.str("device_class")` (pre-dates these subtypes, written for `sensor`/`binary_sensor`) and `addTemplateConfigEntryUpdateFields`'s own copy. Either one alone is enough to make `splitAppliedFields` report a caller-supplied `device_class` as applied. Gate the generic read with `templateSubtypeDomains[entryCtx.entityDomain]` and drop the copy entirely - a doc-only exclusion (`isUpdateExcludedField`) does not stop the value from reaching HA's request.
