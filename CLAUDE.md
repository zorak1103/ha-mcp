...
- generic_thermostat and generic_hygrostat each constrain **two** fields, not one: their actuator field (`heater_entity_id`/`humidifier_entity_id`, must be `switch.*`) AND `target_sensor_entity_id` (must be `sensor.*`) - `helperTypeMetadata.sourceEntities` is a `[]sourceEntityConstraint` (not a single field/domain pair) specifically so both constraints on these two types are checked, not just the first
- Enforced on both `create` (`checkSourceEntityDomain`) and `update` (`checkUpdateSourceEntityDomain`, which resolves the real integration platform via the entity registry since `update`'s `ParseHelperEntityID` only recovers the entity domain) - a mismatch fails with an actionable wrapper recipe (`wrapperRecipeFor`) naming the required domain, rather than surfacing HA's opaque config-flow error
...
- **`manage_helper` update field docs can silently drift from what the update builders actually read:** `helperTypeMetadata.updatableFieldNames()` (used both by `mergeCurrentHelperState()`'s field-preservation loop and by `manage_helper`'s generated per-type description) is `requiredFields ∪ optionalFields` minus `updateExcludedFields` - names that appear in a type's create-time fields but are never consumed by the update path. Three found so far, each for a different reason: `entity_id` (collides with update's own "which helper are we updating" identifier - never forwarded, see the `buildConfigEntryUpdateConfig()` gotcha above), `filter` singular (the filter-type selector is read only by the create-time builder; update only ever reads plural `filters`), and `min_max`'s `type` (the min/max/mean calculation selector - `addExtendedConfigEntryFields`'s min_max section forwards only `entity_ids`/`round_digits`; `type` isn't wired up on update at all, a real gap rather than a deliberate exclusion). `TestUpdatableFields_AreActuallyReadByUpdatePath` (`helpers_consolidated_test.go`) round-trips every remaining `updatableFieldNames()` entry through the real update builder for every helper type, so a field that stops being read on update fails a test instead of silently drifting from the generated docs - add newly-discovered gaps to `updateExcludedFields`, don't just special-case the test.
- **`min_max`'s create-time `type` field shares its args key with the top-level helper-type selector:** `manage_helper create` reads `args["type"]` twice for different purposes - once at the top of `handleCreate` to pick which helper type to build (e.g. `"min_max"`), and again inside `buildMinMaxConfig` to read the min/max/mean calculation selector for that same args map. Since both reads target the literal key `"type"`, creating a `min_max` helper with a `type` field (e.g. `type: "mean"`) is API-shaped to collide with the top-level `type: "min_max"` selector already consumed earlier in `handleCreate` - flagged during the #161/#148 hardening pass but not yet reproduced against live HA or fixed; needs verification before relying on `min_max`'s calculation-type field at create time.
